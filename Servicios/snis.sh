...
echo "SNIS: Actualizando paquetes"
...
apt-get -y install nis --no-install-recommends &> /dev/null && echo "SNIS: NIS instalado" || echo "SNIS: Error al instalar NIS"
...
echo $DOMINIO > /etc/defaultdomain && echo "SNIS: Configurado nombre de dominio como \"$DOMINIO\""
...
echo "SNIS: Ejecutando ypinit"
EOF 2> /dev/null | /usr/lib/yp/ypinit -m &> /dev/null && echo "SNIS: ypinit ejecutado" || echo "SNIS: No se ha podido ejecutar correctamente ypinit"
#Reiniciamos el servicio
echo "SNIS: Reiniciando el servicio"
/etc/init.d/nis restart &> /dev/null && echo "SNIS: Servicio reiniciado" || echo "SNIS: No se ha podido reiniciar el servicio"
